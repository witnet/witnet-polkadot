...
use crate::{
    mock::{ExtBuilder, MAX_WITNET_BYTE_SIZE, Origin, System, Test, Witnet},
    prelude::*,
};
...
fn test_post_request() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_ok!(post);
        System::assert_last_event(WitnetEvent::<Test>::PostedRequest { request_id: 0, sender: 7 }.into() );

        // A second request should get a different ID
        let post = post_dummy_request(account_seven.clone());
        assert_ok!(post);
        System::assert_last_event(WitnetEvent::<Test>::PostedRequest { request_id: 1, sender: 7 }.into() );
    });
}
...
fn test_report_result() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        let report = Witnet::report_result(account_seven.clone(), 0, 999, [0; 32], vec![0; max_byte_size]);
        ...
        let report = Witnet::report_result(genesis_operator.clone(), 0, 1000, [0; 32], vec![0; max_byte_size]);
        ...
        let report = Witnet::report_result(genesis_operator.clone(), 0, 999, [0; 32], vec![0; max_byte_size + 1]);
        let expected = Err(WitnetError::<Test>::OversizedResult.into());
        assert_eq!(report, expected);

        // This should fail because the request is unknown
        let report = Witnet::report_result(genesis_operator.clone(), 1, 999, [0; 32], vec![0; max_byte_size]);
        let expected = Err(WitnetError::<Test>::UnknownRequest.into());
        assert_eq!(report, expected);

        // This should work!
        let report = Witnet::report_result(genesis_operator.clone(), 0, 999, [0; 32], vec![0; max_byte_size]);
        assert_ok!(report);
        System::assert_last_event(WitnetEvent::<Test>::PostedResult { request_id: 0, sender: 5 }.into() );
...
fn test_operators() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_ok!(add);
        System::assert_last_event(WitnetEvent::<Test>::AddedOperator { added_operator: 7, added_by: 5 }.into() );

        // The recently added operator should now be able to report
        let report = Witnet::report_result(account_seven.clone(), 0, 999, [0; 32], vec![0; max_byte_size]);
        assert_ok!(report);
        System::assert_last_event(WitnetEvent::<Test>::PostedResult { request_id: 0, sender: 7 }.into() );

        // The recently added operator should now be able to add another operator
        let add = Witnet::add_operator(account_seven.clone(), 9);
        assert_ok!(add);
        System::assert_last_event(WitnetEvent::<Test>::AddedOperator { added_operator: 9, added_by: 7 }.into() );

        // This second new operator should also be able to report
        let report = Witnet::report_result(account_nine.clone(), 1, 999, [0; 32], vec![0; max_byte_size]);
        assert_ok!(report);
        System::assert_last_event(WitnetEvent::<Test>::PostedResult { request_id: 1, sender: 9 }.into() );
        ...
        assert_ok!(remove);
        System::assert_last_event(WitnetEvent::<Test>::RemovedOperator { removed_operator: 7, removed_by: 5 }.into() );

        // The recently removed operator should no longer be able to report
        let report = Witnet::report_result(account_seven.clone(), 2, 999, [0; 32], vec![0; max_byte_size]);
        let expected = Err(WitnetError::<Test>::UnauthorizedOperator.into());
        assert_eq!(report, expected);

        // But the third operator (account #9) should still be able to report
        let report = Witnet::report_result(account_nine.clone(), 2, 999, [0; 32], vec![0; max_byte_size]);
        assert_ok!(report);
        System::assert_last_event(WitnetEvent::<Test>::PostedResult { request_id: 2, sender: 9 }.into() );
